...
export default function DirectRegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const response = await fetch('/api/test-direct-register', {
        method: 'POST',
        headers: {
      ...
      <div className="max-w-md w-full space-y-8">
        <div>
          ...
          <p className="mt-2 text-center text-sm text-gray-600">
            Direct Database Registration (Bypasses Supabase Auth)
          </p>
          <div className="mt-2 bg-yellow-50 p-2 rounded text-center text-yellow-800 text-sm">
            Use this method if regular registration is disabled
          </div>
        </div>
